...
def dados_aluno_integral_1serie(request, id_aluno_turma):
    urls = str(settings.BASE_DIR)

    # Abre o arquivo pelo caminho do documento.
    wb = load_workbook(filename= urls + '/aplicacoes/dinem/relatorios/ano_letivo_2022/turma_1_serie/relatorio_aluno_integral_1serie_2022.xlsx')

    # Identifaca a aba da planilha pelo nome.
    sh = wb['Histórico-INTEGRAL-2022']
    ...
    resultados = []
    serie = 1
    for relatorio in [relatorio1.resultado]:
    ...
    sh.cell(row= 29, column= 4).value = (sh.cell(row= 29, column= 4).value).replace('relatorio1.protagonismo', relatorio1.protagonismo)
    sh.cell(row= 29, column= 4).font = styles.Font(name= 'Calibri', size= 9)

    sh.cell(row= 32, column= 4).value = (sh.cell(row= 32, column= 4).value).replace('relatorio1.pj_vd', relatorio1.projeto_vida)
    sh.cell(row= 32, column= 4).font = styles.Font(name= 'Calibri', size= 9)

    sh.cell(row= 33, column= 4).value = (sh.cell(row= 33, column= 4).value).replace('relatorio1.ofic_ling', relatorio1.oficina_linguagens)
    sh.cell(row= 33, column= 4).font = styles.Font(name= 'Calibri', size= 9)

    sh.cell(row= 34, column= 4).value = (sh.cell(row= 34, column= 4).value).replace('relatorio1.ofic_mat', relatorio1.oficina_matematica)
    sh.cell(row= 34, column= 4).font = styles.Font(name= 'Calibri', size= 9)

    sh.cell(row= 35, column= 4).value = (sh.cell(row= 35, column= 4).value).replace('relatorio1.ofic_natureza', relatorio1.oficina_natureza)
    sh.cell(row= 35, column= 4).font = styles.Font(name= 'Calibri', size= 9)

    sh.cell(row= 36, column= 4).value = (sh.cell(row= 36, column= 4).value).replace('relatorio1.ofic_humanas', relatorio1.oficina_humanas)
    sh.cell(row= 36, column= 4).font = styles.Font(name= 'Calibri', size= 9)



    sh.cell(row= 51, column= 5).value = (sh.cell(row= 51, column= 5).value).replace('NOME DA ESCOLA', endereco.escola.nome_escola)
    sh.cell(row= 51, column= 5).alignment = styles.Alignment(horizontal='center', vertical= 'center')
    sh.cell(row= 51, column= 5).font = styles.Font(name= 'Carlito', size= 9)

    sh.cell(row= 51, column= 5).alignment = styles.Alignment(horizontal='center', vertical= 'center')

    sh.cell(row= 51, column= 10).value = (sh.cell(row= 51, column= 10).value).replace(f'relatorio1.resultado', resultados[0])
    sh.cell(row= 51, column= 10).alignment = styles.Alignment(horizontal='center', vertical= 'center')

        


    response = HttpResponse(content_type='application/vnd.ms-excel')
    response['Content-Disposition'] = 'attachment; filename="Aluno Integral 1ª Série.xlsx"'
