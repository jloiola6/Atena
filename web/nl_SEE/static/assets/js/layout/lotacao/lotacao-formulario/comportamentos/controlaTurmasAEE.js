// Script para inicializar a sessão do formulário responsável pela enturmação de um Assistente AEE ou Mediador

(() => {
    const container = document.querySelector('[data-form-container="enturmacao-aee"]')

    const selecaoTurma = container.querySelector('[data-selecao-turma-aee]')

    const linkFormularioTurma = container.querySelector('[data-link-formulario-turma-aee]')
    ...
    linkFormularioTurma.addEventListener('click', abrirModalTurma)
    ...
    const adicionaOpcaoTurma = (valor) => {
        const html = `
            <option value="${contador}nova_turma_aee">${valor}</option>
        `

        selecaoTurma.innerHTML += html
        selecaoTurma.selectedIndex = selecaoTurma.options.length - 1
    }
