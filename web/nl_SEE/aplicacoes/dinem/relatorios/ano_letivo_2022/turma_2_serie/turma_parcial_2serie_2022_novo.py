...
def cabecalho_por_pagina(l, sh):
    #Linha 1 Cabeçalho
        ...
        sh.cell(row= l, column= 6).font = styles.Font(bold=True, name= 'Calibri', size= 12)
        ...
        sh.cell(row= l, column= 26).alignment = styles.Alignment(wrap_text= True, horizontal='center', vertical= 'center')
        ...
        sh.row_dimensions[l].height = 115
...
def turma_parcial_2serie(request):
    ...
    wb = load_workbook(filename= str(settings.BASE_DIR) + '/aplicacoes/dinem/relatorios/ano_letivo_2022/turma_2_serie/turma_parcial_2serie_2022_novo.xlsx')
    ...
    for relatorio in relatorios:
        ...
        if relatorio.resultado == 'TRANSFERIDO':
            ...
            sh.cell(row= l, column= 24).value = relatorio.matematica_tecnologias
            sh.cell(row= l, column= 25).value = relatorio.ciencias_natureza
            sh.cell(row= l, column= 26).value = relatorio.formacao_tecnica
