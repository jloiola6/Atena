...
def cabecalho_por_pagina(l, sh):
    #Linha 1 Cabeçalho
        ...
        sh.cell(row= l, column= 30).value = sh.cell(row= 4, column= 30).value
        sh.cell(row= l, column= 30).font = styles.Font(bold=True, name= 'Calibri', size= 12)
        sh.cell(row= l, column= 30).alignment = styles.Alignment(textRotation=90, horizontal='center', vertical= 'center')
        ...
        sh.cell(row= l, column= 29).value = sh.cell(row= 5, column= 29).value
        sh.cell(row= l, column= 29).font = styles.Font(bold=True, name= 'Calibri', size= 12)
        sh.cell(row= l, column= 29).alignment = styles.Alignment(wrap_text= True, horizontal='center', vertical= 'center')
        ...
        #Diversificada
        sh.merge_cells(f'R{l-1}:AB{l-1}')
        #Itinerarios
        sh.merge_cells(f'R{l-2}:AC{l-2}')
        #Formação Tecnica
        sh.merge_cells(f'AC{l-1}:AC{l-1}')
        #Resultado
        sh.merge_cells(f'AD{l-2}:AD{l}')

        l += 1

def turma_integral_1serie(request):
    ...
    relatorios = RelatorioFinal.objects.filter(aluno__in= id_aluno, etapa= '1ª Série').order_by('aluno__nome')

    wb = load_workbook(filename= str(settings.BASE_DIR) + '/aplicacoes/dinem/relatorios/ano_letivo_2022/turma_1_serie/turma_integral_1serie_2022.xlsx')
    sh = wb['Relatório Final-1ª Integral']
    ...
    #Dados da turma
    sh.cell(row= 3, column= 5).value = (sh.cell(row= 3, column= 5).value).replace('variavel_serie', '1ª Série')
    ...
    for relatorio in relatorios:
        ...
        if relatorio.resultado == 'TRANSFERIDO':
            ...
            sh.cell(row= l, column= 18).value = relatorio.eletiva
            sh.cell(row= l, column= 19).value = relatorio.estudo_orientado
            sh.cell(row= l, column= 20).value = relatorio.praticas_experimentais
            sh.cell(row= l, column= 21).value = relatorio.protagonismo
            sh.cell(row= l, column= 22).value = '-'
            sh.cell(row= l, column= 23).value = '-'
            sh.cell(row= l, column= 24).value = relatorio.projeto_vida
            sh.cell(row= l, column= 25).value = relatorio.oficina_linguagens
            sh.cell(row= l, column= 26).value = relatorio.oficina_matematica
            sh.cell(row= l, column= 27).value = relatorio.oficina_natureza
            sh.cell(row= l, column= 28).value = relatorio.oficina_humanas
            sh.cell(row= l, column= 29).value = relatorio.formacao_tecnica
            ...
            if relatorio.resultado == 'APROVADO':
                ...
                resultado = 'RP'
            sh.cell(row= l, column= 30).value = resultado
    ...
    response['Content-Disposition'] = 'attachment; filename="Turma Integral 1ª Série.xlsx"'
