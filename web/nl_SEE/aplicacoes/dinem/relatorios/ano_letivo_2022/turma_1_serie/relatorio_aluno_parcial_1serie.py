...
def dados_aluno_parcial_1serie(request, id_aluno_turma):
    urls = str(settings.BASE_DIR)

    # Abre o arquivo pelo caminho do documento.
    wb = load_workbook(filename= urls + '/aplicacoes/dinem/relatorios/ano_letivo_2022/turma_1_serie/relatorio_aluno_parcial_1serie_2022.xlsx')

    # Identifaca a aba da planilha pelo nome.
    sh = wb['Histórico - NEM Parcial -2022']
    ...
    sh.cell(row= 2, column= 1).value = (sh.cell(row= 2, column= 1).value).replace('NOME DA ESCOLA', endereco.escola.nome_escola)
    ...
    sh.cell(row= 45, column= 5).value = (sh.cell(row= 45, column= 5).value).replace('NOME DA ESCOLA', endereco.escola.nome_escola)
    sh.cell(row= 45, column= 5).alignment = styles.Alignment(horizontal='center', vertical= 'center')
    sh.cell(row= 45, column= 5).font = styles.Font(name= 'Carlito', size= 9)

    sh.cell(row= 45, column= 5).alignment = styles.Alignment(horizontal='center', vertical= 'center')

    sh.cell(row= 45, column= 10).value = (sh.cell(row= 45, column= 10).value).replace(f'relatorio1.resultado', resultados[0])
    sh.cell(row= 45, column= 10).alignment = styles.Alignment(horizontal='center', vertical= 'center')


    response = HttpResponse(content_type='application/vnd.ms-excel')
    response['Content-Disposition'] = 'attachment; filename="Aluno Parcial 1ª Série.xlsx"'
