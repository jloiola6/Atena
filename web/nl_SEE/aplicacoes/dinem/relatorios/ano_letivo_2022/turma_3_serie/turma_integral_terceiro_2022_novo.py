...
def cabecalho_por_pagina(l, sh):
    #Linha 1 Cabeçalho
        ...
        sh.cell(row= l, column= 32).value = sh.cell(row= 4, column= 32).value
        sh.cell(row= l, column= 32).font = styles.Font(bold=True, name= 'Calibri', size= 12)
        sh.cell(row= l, column= 32).alignment = styles.Alignment(textRotation=90, horizontal='center', vertical= 'center')
        ...
        sh.cell(row= l, column= 27).value = sh.cell(row= 5, column= 27).value
        sh.cell(row= l, column= 27).font = styles.Font(bold=True, name= 'Calibri', size= 12)
        sh.cell(row= l, column= 27).alignment = styles.Alignment(wrap_text= True, horizontal='center', vertical= 'center')
        ...
        #Diversificada
        sh.merge_cells(f'Q{l-1}:Z{l-1}')
        #Itinerarios
        sh.merge_cells(f'Q{l-2}:AE{l-2}')
        #Formação Tecnica
        sh.merge_cells(f'AE{l-1}:AE{l-1}')
        #Resultado
        sh.merge_cells(f'AF{l-2}:AF{l}')
        #Rotas
        sh.merge_cells(f'AA{l-1}:AD{l-1}')
...
def turma_integral_terceiro(request):
    ...
    wb = load_workbook(filename= str(settings.BASE_DIR) + '/aplicacoes/dinem/relatorios/ano_letivo_2022/turma_3_serie/turma_integral_terceiro_2022_novo.xlsx')
    ...
    sh.cell(row= 3, column= 18).value = (sh.cell(row= 3, column= 18).value).replace('variavel_turno', turma.turno)
    ...
    for relatorio in relatorios:
        ...
        if relatorio.resultado == 'TRANSFERIDO':
            ...
            sh.cell(row= l, column= 27).value = relatorio.linguagens_tecnologias
            sh.cell(row= l, column= 28).value = relatorio.ciencias_humanas
            sh.cell(row= l, column= 29).value = relatorio.matematica_tecnologias
            sh.cell(row= l, column= 30).value = relatorio.ciencias_natureza
