...
def cabecalho_por_pagina(l, sh):
    #Linha 1 Cabeçalho
        ...
        sh.cell(row= l, column= 22).value = sh.cell(row= 4, column= 22).value
        sh.cell(row= l, column= 22).font = styles.Font(bold=True, name= 'Calibri', size= 12)
        sh.cell(row= l, column= 22).alignment = styles.Alignment(textRotation=90, horizontal='center', vertical= 'center')
        ...
        #Itinerarios
        sh.merge_cells(f'R{l-2}:U{l-1}')
        #Resultado
        sh.merge_cells(f'V{l-2}:V{l}')

        l += 1


def turma_parcial_1serie(request):
    ...
    wb = load_workbook(filename= str(settings.BASE_DIR) + '/aplicacoes/dinem/relatorios/ano_letivo_2022/turma_1_serie/turma_parcial_1serie_2022.xlsx')
    sh = wb['Relatório Final-Parcial-1ª']

    def cabecalho():
        sh.cell(row= l, column= 1).value = sh.cell(row= 4, column= 1)

    #Dados da escola
    sh.cell(row= 1, column= 9).value = (sh.cell(row= 1, column= 9).value).replace('NOME DA ESCOLA', endereco.escola.nome_escola)
    sh.cell(row= 1, column= 9).font = styles.Font(bold=True, name= 'Calibri', size= 14)
    sh.cell(row= 1, column= 9).value = (sh.cell(row= 1, column= 9).value).replace('Endereco', endereco.rua)
    if endereco.cep != '':
        sh.cell(row= 1, column= 9).value = (sh.cell(row= 1, column= 9).value).replace('CEP', endereco.cep)
    else:
        sh.cell(row= 1, column= 9).value = (sh.cell(row= 1, column= 9).value).replace('-CEP', '')

    sh.cell(row= 1, column= 9).value = (sh.cell(row= 1, column= 9).value).replace('Municipio', endereco.municipio)
    sh.cell(row= 1, column= 9).value = (sh.cell(row= 1, column= 9).value).replace('Telefone', contato[0].contato)
    sh.cell(row= 1, column= 9).value = (sh.cell(row= 1, column= 9).value).replace('email', email[0].contato)
    ...
    for relatorio in relatorios:
        ...
        if relatorio.resultado == 'TRANSFERIDO':
            ...
            sh.cell(row= l, column= 7).value = relatorio.ingles
            sh.cell(row= l, column= 8).value = relatorio.ed_fisica
            sh.cell(row= l, column= 9).value = relatorio.arte
            ...
            if relatorio.resultado == 'APROVADO':
                ...
                resultado = 'RP'
            sh.cell(row= l, column= 22).value = resultado
    ...
    sh.merge_cells(f'G{l+2}:N{l+2}')

    sh.cell(row= l+2, column= 15).value = sh2.cell(row= 2, column= 15).value
    sh.cell(row= l+2, column= 15).alignment = styles.Alignment(horizontal='left', vertical= 'top')
    sh.merge_cells(f'O{l+2}:V{l+2}')
    ...
    response['Content-Disposition'] = 'attachment; filename="Turma Parcial 1ª Série.xlsx"'
